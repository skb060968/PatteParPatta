/**
 * Patte Par Patta Firebase Sync Module
 *
 ...
 *
 * All data is stored under `ppp-rooms/` (separate from `rooms/` and `tambola-rooms/`).
 */
...
import { serializeCard, deserializeCard } from './deck.js';
...
export async function firebaseRetry(fn, maxRetries = 2, delayMs = 500) {
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`⚠️ Firebase retry ${attempt + 1}/${maxRetries}:`, err.message);
 ...
 * Rejects if the room is not in lobby status or is full (4 players).
...
export async function joinRoom(roomCode, playerName, playerEmoji) {
  ...
  await firebaseRetry(() =>
    update(ref(db, `ppp-rooms/${roomCode}`), {
      [`players/${playerKey}`]: {
        name: playerName,
        emoji: playerEmoji,
        uid: uid,
 ...
 */
export async function writeThrow(roomCode, playerIndex, thrownCard, captured, newGameState) {
...
export function setupDisconnectHandler(roomCode, playerIndex) {
  const connectedRef = ref(
    db,
    `ppp-rooms/${roomCode}/players/player_${playerIndex}/connected`
  );
...
export async function endRoom(roomCode) {
  await firebaseRetry(() =>
    update(ref(db, `ppp-rooms/${roomCode}/meta`), {
      status: 'ended',
      lastActivity: Date.now(),
    })
  );
